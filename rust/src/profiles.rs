...
pub fn hrc32_v1(key_bytes: &[u8], key_id: &str) -> Profile {
    frozen("hrc32-v1", 1, vec![3, 3, 1], key_bytes, key_id)
}

/// Frozen profile `hrc32s-v1`: 6 body + 2 checksum, feistel-v1 permutation,
/// 8 rounds. Self-service use; provably detects all single-symbol
/// substitutions and all adjacent transpositions (spec 6.3).
pub fn hrc32s_v1(key_bytes: &[u8], key_id: &str) -> Profile {
    frozen("hrc32s-v1", 2, vec![3, 3, 2], key_bytes, key_id)
}
